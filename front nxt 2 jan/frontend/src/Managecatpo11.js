...
function ManageVaccine() {
  ...
  useEffect(() => {
    const fetchVaccines = async () => {
      try {
        const response = await axios.get('http://amrithaahospitals.visualplanetserver.in/vaccine');
  ...
  const handleUpdateVaccine = async () => {
    try {
      await axios.put(`http://amrithaahospitals.visualplanetserver.in/vaccine/${selectedVaccine.id}`, {
  ...
  const handleDeleteVaccine = async (vaccine) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://amrithaahospitals.visualplanetserver.in/vaccine/${vaccine.id}`);
