...
const BillingFrom = () => {
    ...
    const fetchapidata = async () => {
        try {
            ...
            const visit = searchParams.get('visited')
            const phonenumber = searchParams.get('businessname')
            ...
            if (api.service && Array.isArray(api.service)) {
                const updatedServiceArray = api.service.map(({ service_name, price, discount, ...rest }) => ({
                    ...rest,
            ...
            const formattedAdvance = adv.data.result.map(item => ({
                ...item,
                amount: item.advance,
                date: item.date_paid.split('T')[0]
            }));
    ...
    useEffect(() => {
        ...
        const totalAdvance = advance.reduce(
            (sum, item) => sum + Number(item.amount || 0),
            0
        )
        setBalance(finalTotal - totalAdvance)
        console.log('balance', balance)
    }, [services, overallDiscount, membershipPrice, membershipOffer, advance]);
    ...
    const generatePDF = () => {
        ...
        const validServices = services.filter(
            (service) => service.service.trim() || service.price || service.discount
        );
    ...
    const handleSubmit = async (status) => {
        console.log(status)
        const searchParams = new URLSearchParams(location.search);
        const finaladvance = advanceInput.trim() != "" ? [...advance, { amount: advanceInput, date: new Date().toISOString().split("T")[0], method: paymentMode }] : [...advance]
        const data = {
        ...
        console.log("validate Service=>", validServices)
        ...
        const billingData = {
            userId: basic.id,
            userName: basic.full_name,
            phoneNumber: basic.phone_number,
            visitNumber: basic.visted,
            nurseName: basic.nursename,
        ...
        try {
            ...
            if (status == 'Final_Bill') {
                ...
                // http://localhost:5173/BillingForm?user=asdf&businessname=5566778899&name=asdf&id=A00F18916&visited=1&MemberType=null&belongedlocation=karur
                navigate(`/billing/${type}?loginlocation=${urlParams.loginLocation}&franchiselocation=${urlParams.belongedlocation}`, { replace: true });
            }
        } catch (error) {
    ...
    return (<>
        <div className={style.billing_form_container}>
                ...
                <table className={style.services_table}>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td className={style.input_with_sugestion}>
                                <input
                                    type="text"
            ...
            <div className={style.billing_financial_adjustments}>
                <div className={style.advance_section}>
            ...
            <div className={style.button_container} style={{ marginTop: '2rem' }}>
                <div className={style.balance_container}>
                    <h1 className={style.balance}>RS: {balance}/-</h1>
                </div>
                <div className={style.button_holder}>
                    ...
                    <button className={style.save_billing_button} onClick={() => handleSubmit('Final_Bill')}>Final Bill</button>
                </div>
            </div>
        </div>
    </>)
