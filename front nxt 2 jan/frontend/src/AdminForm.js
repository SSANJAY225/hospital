...
const AdminFormIn = () => {
  ...
  const handleAddNurseName = async (name) => {
    if (name.trim() === '') return;
    try {
      await axios.post('http://amrithaahospitals.visualplanetserver.in/addDoctorName', { doctorName: name });
      setDoctorName(name);
    ...
    try {
      ...
      if (hasDentalData) {
        ...
        console.log("Sending dental data:", {
          url: `http://amrithaahospitals.visualplanetserver.in/adddental/${encodedName}/${encodedVisit}/${encodedPhone}`,
          body: dental
        });
        // console.log(dental)
        ...
        const d = await axios.post('http://amrithaahospitals.visualplanetserver.in/add-dental', { encodedName, encodedPhone, encodedVisit, dental })
        console.log(d)
      }
