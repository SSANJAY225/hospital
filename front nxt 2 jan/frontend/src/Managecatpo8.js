...
function ManageTiming() {
  ...
  useEffect(() => {
    const fetchTimings = async () => {
      try {
        const response = await axios.get('http://amrithaahospitals.visualplanetserver.in/timing');
  ...
  const handleUpdateTiming = async () => {
    try {
      await axios.put(`http://amrithaahospitals.visualplanetserver.in/timing/${selectedTiming.id}`, {
  ...
  const handleDeleteTiming = async (timing) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://amrithaahospitals.visualplanetserver.in/timing/${timing.id}`);
