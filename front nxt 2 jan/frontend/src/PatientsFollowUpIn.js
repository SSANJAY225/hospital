...
function PatientsFollowUpIn() {
  ...
  const handleBusinessClick = (business) => {
    ...
    const basePath = statusFilter === 'completed' ? '/PatientFormCompleted' : '/adminformin';
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/fetch-patients-in?${queryString}`);
...
export default PatientsFollowUpIn;