...
function Login() {
  ...
  return (
    <div className={style.login_page_container}>
      <div className={style.login_main_card}>
        <div className={style.login_brand_section}>
          {/* <img src={myImage} alt="Visual Planet Logo" className="login-brand-logo" /> */}
          <h1 className={style.login_brand_title}>Amirthaa <span style={{ color: 'black' }}>Dental</span></h1>
        </div>
        <form onSubmit={handleSubmit}>
          <div className={style.login_form_group}>
            <label htmlFor="loginLocation" className={style.login_form_label}>User Name</label>
            <input
              type='text'
              name='loginLocation'
            ...
            {showSuggestions && countrySuggestions.length > 0 && (
              <ul className={style.login_suggestions_list}>
                {countrySuggestions.map((suggestion, index) => (
                  <li
                    key={index}
                    className={style.login_suggestion_item}
            ...
            {errors.loginLocation && <span className={style.login_error_message}>{errors.loginLocation}</span>}
          </div>
          <div className={style.login_form_group}>
            <label htmlFor="password" className={style.login_form_label}>Password</label>
            <input
              type="password"
              ...
              className={style.login_form_input}
            />
            {errors.password && <span className={style.login_error_message}>{errors.password}</span>}
          </div>
          <button type='submit' className={style.login_submit_button}>Login</button>
        </form>
      </div>
    </div>
  );
