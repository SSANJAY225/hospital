...
function Managecatpo13() {
  ...
  useEffect(() => {
    const fetchTreatments = async () => {
      try {
        const response = await axios.get('http://amrithaahospitals.visualplanetserver.in/dental');
  ...
  const handleUpdateTreatment = async () => {
    try {
      await axios.put(`http://amrithaahospitals.visualplanetserver.in/dental/${selectedTreatment.id}`, {
  ...
  const handleDeleteTreatment = async (treatment) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://amrithaahospitals.visualplanetserver.in/dental/${treatment.id}`);
          refreshTreatments();
