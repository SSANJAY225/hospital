...
function ManageDoctors() {
  ...
  const fetchDoctorEntries = async () => {
    try {
      const response = await axios.get('https://amrithaahospitals.visualplanetserver.in/doctors_names');
  ...
  const handleUpdateDoctor = async () => {
    ...
    try {
      await axios.put(`https://amrithaahospitals.visualplanetserver.in/doctors_names/${selectedDoctor.id}`, {
        name: newDoctorName,
      });
      await fetchDoctorEntries();
      setModalIsOpen(false);
  ...
  const handleDeleteDoctor = async (doctor) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://amrithaahospitals.visualplanetserver.in/doctors_names/${doctor.id}`);
          await fetchDoctorEntries();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
