...
function Managecatpo12() {
  ...
  useEffect(() => {
    const fetchVaccines = async () => {
      try {
        const response = await axios.get('http://amrithaahospitals.visualplanetserver.in/getRoA');
  ...
  const handleUpdateVaccine = async () => {
    try {
      ...
      await axios.put(`http://amrithaahospitals.visualplanetserver.in/updateRoA/${selectedVaccine}`, {
  ...
  const handleDeleteVaccine = async (vaccine) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://amrithaahospitals.visualplanetserver.in/deleteRoA/${vaccine}`);
